...
func main() {
	...
	if cfg.WAL.Enabled {
		...
		if events, e := wal.ReadAll(eventLog.Path()); e == nil {
	...
	persist := func(event model.Event) error {
		if eventLog != nil { return eventLog.Append(event) }
		return nil
	}
	...
	onRecovery := func(from, to time.Time) error {
		...
		if len(events) > 0 { log.Printf("journal recovery completed: %d candidate records", len(events)) }
		return nil
	}
	...
	go func() {
		err := systemd.RunResilient(ctx, cfg.Services, cfg.Systemd.ReconnectInterval, onSnapshot,
			...
			func(err error) { log.Printf("systemd D-Bus error: %v", err) },
			onRecovery)

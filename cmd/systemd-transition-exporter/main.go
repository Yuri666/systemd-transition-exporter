...
func main() {
	...
	var mu sync.Mutex
	currentBootID,err:=systemd.BootID();if err!=nil{log.Fatal(err)}
	bootTime,_:=systemd.BootTime()

	emit:=func(e model.Event)error{if eventLog!=nil{if err:=eventLog.Append(e);err!=nil{return err}};reg.Event(e);log.Printf("transition seq=%d service=%s state=%s timestamp_ms=%d source=%d",e.Sequence,e.Service,e.State,e.EventTimeUnixMS,e.Source);return nil}
	onSnapshot:=func(s model.UnitSnapshot)error{
		mu.Lock();defer mu.Unlock()
		if currentBootID!=""&&s.BootID!=currentBootID{for _,e:=range eng.ApplyReboot(s.BootID,bootTime){if err:=emit(e);err!=nil{return err}};currentBootID=s.BootID;bootTime,_=systemd.BootTime()}
	...
	go func(){if err:=systemd.RunResilient(ctx,cfg.Services,cfg.Systemd.ReconnectInterval,onSnapshot);err!=nil&&err!=context.Canceled{log.Printf("systemd monitor stopped: %v",err);stop()}}()

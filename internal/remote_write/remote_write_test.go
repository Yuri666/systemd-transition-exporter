...
func TestCheckpointPersistsAfterSuccessfulBatch(t *testing.T) {
	...
	defer server.Close()
	s := testSender(t, server.URL, filepath.Join(t.TempDir(), "checkpoint.json"))
	if err := s.Send(context.Background(), []model.Event{event(1, "cups.service", model.StateDown, 1000), event(2, "cups.service", model.StateUp, 2000)}); err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	restarted := testSender(t, server.URL, s.cfg.Checkpoint)
	if got := restarted.LastSent(); got != 2 {
		t.Fatalf("restarted LastSent = %d, want 2", got)
	}
}

func TestMigrateCheckpointCopiesLegacyStateOnce(t *testing.T) {
	dir := t.TempDir()
	source := filepath.Join(dir, "remote_write.checkpoint")
	destination := filepath.Join(dir, "remote_write.checkpoint.abc123")
	if err := os.WriteFile(source, []byte("{\"last_sequence\":42}\n"), 0640); err != nil {
		t.Fatal(err)
	}
	if err := MigrateCheckpoint(source, destination); err != nil {
		t.Fatal(err)
	}
	s := testSender(t, "http://127.0.0.1:1", destination)
	if got := s.LastSent(); got != 42 {
		t.Fatalf("migrated LastSent = %d, want 42", got)
	}
	if _, err := os.Stat(source); err != nil {
		t.Fatalf("legacy checkpoint was removed: %v", err)
	}

	if err := os.WriteFile(source, []byte("{\"last_sequence\":99}\n"), 0640); err != nil {
		t.Fatal(err)
	}
	if err := MigrateCheckpoint(source, destination); err != nil {
		t.Fatal(err)
	}
	reloaded := testSender(t, "http://127.0.0.1:1", destination)
	if got := reloaded.LastSent(); got != 42 {
		t.Fatalf("existing destination was overwritten: LastSent = %d", got)
	}

	otherDestination := filepath.Join(dir, "remote_write.checkpoint.different")
	if err := MigrateCheckpoint(source, otherDestination); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(otherDestination); !os.IsNotExist(err) {
		t.Fatalf("legacy checkpoint was migrated to a second target: %v", err)
	}
}
...
func TestRestartResumesFromCheckpointWithoutResendingDeliveredEvents(t *testing.T) {
	...
	if err := first.Send(context.Background(), []model.Event{event(1, "cups.service", model.StateDown, 1000), event(2, "cups.service", model.StateUp, 2000)}); err != nil {
		t.Fatalf("first Send: %v", err)
	}
	second := testSender(t, server.URL, checkpointPath)
	if err := second.Send(context.Background(), []model.Event{event(1, "cups.service", model.StateDown, 1000), event(2, "cups.service", model.StateUp, 2000), event(3, "cups.service", model.StateDown, 3000)}); err != nil {
...
func TestRecoveredStateSamplesPreserveTimestamps(t *testing.T) {
	...
	if err := s.SendRecoveredStates(context.Background(), samples); err != nil {
		t.Fatalf("SendRecoveredStates: %v", err)
	}
	if len(received) != len(samples) {
		t.Fatalf("received samples = %d, want %d", len(received), len(samples))
	}
	for i, want := range samples {
		if received[i] != want.TimestampUnixMS {
			t.Fatalf("sample %d timestamp = %d, want %d", i, received[i], want.TimestampUnixMS)
		}
	}
	if got := s.LastSent(); got != 0 {
		t.Fatalf("LastSent = %d, want 0", got)
...
func TestSendGivesUpAfterBoundedRetriesWhileReceiverIsDown(t *testing.T) {
	...
	if IsPermanent(err) {
		t.Fatalf("error must stay retryable so the caller keeps the batch: %v", err)
	}

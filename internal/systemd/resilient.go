...
func RunResilient(ctx context.Context, services []string, reconnectInterval, reconciliationInterval time.Duration,
	...
	for {
		...
		if !disconnectedAt.IsZero() && onRecovery != nil {
			until := time.Now()
			if err := onRecovery(disconnectedAt, until); err != nil && onConnectionError != nil {
				onConnectionError(fmt.Errorf("journal recovery: %w", err))
			}
		...
		connectedOnce = true
		runErr := m.Run(ctx, reconciliationInterval, func(u *Unit) error {
			s, e := u.Snapshot(bootID)
			if e != nil {
				return e
			}
		...
		if onConnectionState != nil {
			onConnectionState(false, disconnectedAt)
		}

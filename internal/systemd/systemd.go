...
func (d *DBus) Close() error { if d.conn != nil { d.conn.Close() }; return nil }
...
func (d *DBus) LoadUnit(service string) (*Unit, error) {
	call := d.conn.Object(busName, managerPath).Call(managerIface+".LoadUnit", 0, service)
	if call.Err != nil { return nil, fmt.Errorf("LoadUnit(%s): %w", service, call.Err) }
	var path dbus.ObjectPath
	if err := call.Store(&path); err != nil { return nil, fmt.Errorf("decode unit path: %w", err) }
	return &Unit{conn: d.conn, path: path, service: service}, nil
}

func (u *Unit) Snapshot(bootID string) (model.UnitSnapshot, error) {
	obj := u.Object()
	get := func(name string) (interface{}, error) {
		v, err := obj.GetProperty(unitIface+"."+name)
		if err != nil { return nil, fmt.Errorf("get %s: %w", name, err) }
		return v.Value(), nil
	}
	active, err := get("ActiveState"); if err != nil { return model.UnitSnapshot{}, err }
	...
	exit, err := get("ActiveExitTimestamp"); if err != nil { return model.UnitSnapshot{}, err }
	enterMono, err := get("ActiveEnterTimestampMonotonic"); if err != nil { return model.UnitSnapshot{}, err }
	exitMono, err := get("ActiveExitTimestampMonotonic"); if err != nil { return model.UnitSnapshot{}, err }

	as, ok := active.(string); if !ok { return model.UnitSnapshot{}, fmt.Errorf("ActiveState has type %T", active) }
	ss, ok := sub.(string); if !ok { return model.UnitSnapshot{}, fmt.Errorf("SubState has type %T", sub) }
	return model.UnitSnapshot{
		Service: u.service, ActiveState: as, SubState: ss,
		ActiveEnterTimestampUS: asUint64(enter), ActiveExitTimestampUS: asUint64(exit),
		ActiveEnterTimestampMonotonicUS: asUint64(enterMono), ActiveExitTimestampMonotonicUS: asUint64(exitMono),
		BootID: bootID,
	}, nil
}

func asUint64(v interface{}) uint64 {
	switch x := v.(type) { case uint64: return x; case int64: if x >= 0 { return uint64(x) }; case uint32: return uint64(x) }
	return 0
}
...
func (m *Monitor) Subscribe() error {
	...
	obj := m.dbus.conn.Object(dbusIF, dbusPath)
	if err := obj.Call(dbusIF+".AddMatch", 0, args).Err; err != nil { return fmt.Errorf("AddMatch: %w", err) }
	return nil
}
...
func (m *Monitor) Run(ctx context.Context, handler func(*Unit) error) error {
	...
	for {
		...
		case sig := <-signals:
			if sig == nil || sig.Name != propertiesIF+".PropertiesChanged" { continue }
			u, ok := m.byPath[sig.Path]; if !ok || len(sig.Body) < 2 { continue }
			...
			if err := handler(u); err != nil { return err }
		}
	}
}

func interesting(props map[string]dbus.Variant) bool {
	for name := range props { switch name {
	case "ActiveState", "SubState", "ActiveEnterTimestamp", "ActiveExitTimestamp", "ActiveEnterTimestampMonotonic", "ActiveExitTimestampMonotonic": return true
	} }
	return false
}

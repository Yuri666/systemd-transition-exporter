...
func (u *Unit) Snapshot(bootID string) (model.UnitSnapshot, error) {
	...
	return model.UnitSnapshot{Service:u.service, ActiveState:as, SubState:ss, ActiveEnterTimestampUS:asUint64(enter), ActiveExitTimestampUS:asUint64(exit), ActiveEnterTimestampMonotonicUS:asUint64(enterMono), ActiveExitTimestampMonotonicUS:asUint64(exitMono), BootID:bootID, ObservedAt:time.Now()}, nil
}
...
func (m *Monitor) Subscribe() error {
	...
	if err := m.dbus.conn.Object(dbusIF, dbusPath).Call(dbusIF+".AddMatch", 0, args).Err; err != nil { return fmt.Errorf("AddMatch: %w", err) }
	return nil
}

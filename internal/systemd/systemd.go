...
const (
	busName="org.freedesktop.systemd1"; managerPath=dbus.ObjectPath("/org/freedesktop/systemd1"); managerIface="org.freedesktop.systemd1.Manager"; unitIface="org.freedesktop.systemd1.Unit"; propertiesIF="org.freedesktop.DBus.Properties"; dbusIF="org.freedesktop.DBus"; dbusPath=dbus.ObjectPath("/org/freedesktop/DBus")
)
...
func BootTime()(time.Time,error){b,err:=os.ReadFile("/proc/uptime");if err!=nil{return time.Time{},fmt.Errorf("read uptime: %w",err)};f:=strings.Fields(string(b));if len(f)==0{return time.Time{},fmt.Errorf("invalid /proc/uptime")};sec,err:=strconv.ParseFloat(f[0],64);if err!=nil{return time.Time{},fmt.Errorf("parse uptime: %w",err)};return time.Now().Add(-time.Duration(sec*float64(time.Second))),nil}

type Unit struct{conn *dbus.Conn;path dbus.ObjectPath;service string}
func(u *Unit)Path()dbus.ObjectPath{return u.path};func(u *Unit)Service()string{return u.service};func(u *Unit)Object()dbus.BusObject{return u.conn.Object(busName,u.path)}
func(d *DBus)LoadUnit(service string)(*Unit,error){c:=d.conn.Object(busName,managerPath).Call(managerIface+".LoadUnit",0,service);if c.Err!=nil{return nil,fmt.Errorf("LoadUnit(%s): %w",service,c.Err)};var p dbus.ObjectPath;if err:=c.Store(&p);err!=nil{return nil,fmt.Errorf("decode unit path: %w",err)};return &Unit{conn:d.conn,path:p,service:service},nil}
func(u *Unit)Snapshot(bootID string)(model.UnitSnapshot,error){obj:=u.Object();get:=func(n string)(interface{},error){v,e:=obj.GetProperty(unitIface+"."+n);if e!=nil{return nil,fmt.Errorf("get %s: %w",n,e)};return v.Value(),nil};a,e:=get("ActiveState");if e!=nil{return model.UnitSnapshot{},e};s,e:=get("SubState");if e!=nil{return model.UnitSnapshot{},e};en,e:=get("ActiveEnterTimestamp");if e!=nil{return model.UnitSnapshot{},e};ex,e:=get("ActiveExitTimestamp");if e!=nil{return model.UnitSnapshot{},e};enm,e:=get("ActiveEnterTimestampMonotonic");if e!=nil{return model.UnitSnapshot{},e};exm,e:=get("ActiveExitTimestampMonotonic");if e!=nil{return model.UnitSnapshot{},e};as,ok:=a.(string);if !ok{return model.UnitSnapshot{},fmt.Errorf("ActiveState has type %T",a)};ss,ok:=s.(string);if !ok{return model.UnitSnapshot{},fmt.Errorf("SubState has type %T",s)};return model.UnitSnapshot{Service:u.service,ActiveState:as,SubState:ss,ActiveEnterTimestampUS:asUint64(en),ActiveExitTimestampUS:asUint64(ex),ActiveEnterTimestampMonotonicUS:asUint64(enm),ActiveExitTimestampMonotonicUS:asUint64(exm),BootID:bootID,ObservedAt:time.Now()},nil}

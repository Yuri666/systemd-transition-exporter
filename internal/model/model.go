...
// CanonicalBootID normalizes the two boot identifier spellings used on Linux.
// /proc/sys/kernel/random/boot_id is hyphenated while the journal's _BOOT_ID
// field is not, so they must be normalized before any comparison.
func CanonicalBootID(id string) string {
